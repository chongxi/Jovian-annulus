...
void wrap_scene_model();

typedef std::vector< std::string > Name_List;
typedef std::map< std::string, std::vector< btRigidBody* > > Segment_Map;
/**
...
class Scene_Model
{
  ...
  public:
    ...
    std::vector< osg::MatrixTransform* > get_light_markers() { return _light_marker[ _active_model ]; }
    std::vector< osg::MatrixTransform* > get_light_transforms() { return _light_transform[ _active_model ]; }
    /**
     ...
     * than expected as OSG computes the bound of a scene as a bounding sphere that
    ...
    Name_List start_objects() const;
    Name_List objects() const;
    Name_List segments() const;
    ///@}
    ...
    int current_shader() const { return _current_shader; }
    ...
    bool has_segments() const { return _segments_set[ _active_model ]; }
    ///@}
    ...
    void set_attenuation( float constant, float linear, float quad );
    ...
    void set_diffuse_color( osg::Vec4 diff, size_t index );
    void set_diffuse_power( float d_power, size_t index  );
    ...
    void update_segment_radii( float inner, float outer ) { _inner = inner; _outer = outer; }
    void set_world_unit_scale( float factor ) { _scale_factor = factor; }
    ///@}
    ...
    ///@{
    void apply_physics( std::string name, float x, float y, float z );
    ...
    void move_object( std::string name, float x, float y, float z );
    void move_offset( float x, float y, float z );
    void set_rotations( float x, float y, float z );
    void move_to_position( float x, float y, float z, float deg );
    void offset_object( std::string name, float x, float y, float z );
    ...
    void set_alpha( std::string name, float alpha );
    void reset_traversal() { _camera_callback[ _active_model ]->reset(); }
    void reset_motion();
    void save_scene_graph() { osgDB::writeNodeFile(*_root, "jovian.osgt"); }
    ...
    void toggle_segments( boost::python::list& seg_ids, bool on_or_off );
    ///@}

    /// @name Miscellaneous
    ///@{
    void add_interaction_handler( Viewing_Window_Qt* v);
  ...
  protected:
    ...
    void create_lights( osg::BoundingBox const* bb, osg::StateSet* root_state_set );
    void process_channel( std::vector< Node_Offset_Pair >::iterator& itn );
    void process_start( std::vector< Node_Offset_Pair >::iterator& itn );
    std::vector< btRigidBody* > process_segment( std::vector< Node_Offset_Pair >::iterator& itn );
  ...
  private:
    ...
    osg::Program* _per_vertex_one_sided_program;
    osg::Program* _per_pixel_program;
    osg::Program* _per_pixel_one_sided_program;
    osg::Shader* _per_vertex_vert_shader;
    osg::Shader* _per_vertex_one_sided_vert_shader;
    osg::Shader* _per_vertex_frag_shader;
    osg::Shader* _per_pixel_vert_shader;
    osg::Shader* _per_pixel_one_sided_vert_shader;
    osg::Shader* _per_pixel_frag_shader;
    osg::Shader* _per_pixel_one_sided_frag_shader;
    osg::StateSet* _per_vertex_state_set;
    osg::StateSet* _per_vertex_one_sided_state_set;
    osg::StateSet* _per_pixel_state_set;
    osg::StateSet* _per_pixel_one_sided_state_set;
    ...
    std::vector< osg::Group* > _light_group;
    std::vector< std::vector< osg::MatrixTransform* > > _light_marker;
    std::vector< std::vector< osg::LightSource* > > _light_source;
    std::vector< std::vector< osg::MatrixTransform* > > _light_transform;
    osg::Vec4 _ambient, _diffuse, _specular;
    float _power;
    int _light_rows, _light_cols, _max_lights;
    float _light_height;
    float _constant_attenuation, _linear_attenuation, _quadratic_attenuation;
    ...
    std::vector< std::map< std::string, std::vector< btRigidBody* > > > _segments;
    ...
    std::vector< bool > _eye_pt_set, _path_set, _segments_set;
    ...
    osg::Group* launchHandlerAttachPoint;
    // For controlling segment influence
    std::vector< std::map<std::string, bool> > _segment_status;
