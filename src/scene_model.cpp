                      ...
                      int index1 )
{
    ...
    if ( ps0 && ps1 && ( ps0->camera || ps1->camera ) )
    {
        ...
        if ( ps0->segment || ps1->segment )
        {
            //std::cout << "Contact! " << ps0->node->getName() << ", " << ps1->node->getName() << std::endl;
            if ( !ps0->camera )
                contactCallback.addSegmentNode( ps0->s_node->name, ps0->s_node );
            else
                contactCallback.addSegmentNode( ps1->s_node->name, ps1->s_node );
        }
    }
...
osg::Material*
createSimpleMaterial( osg::Vec4 color )
{
    ...
    _contactCallback( &contactCallback ), _open_field_turning( false )
{
    ...
    _ambient = osg::Vec4( 0.1f, 0.1f, 0.1f, 1.f );
    _diffuse = osg::Vec4( 0.1f, 0.1f, 0.1f, 1.f );
    _specular = osg::Vec4( 0.0f, 0.0f, 0.0f, 1.f );
    ...
    _per_vertex_state_set->addUniform( new osg::Uniform( "numLights", 8 ) );
    for ( int i = 0; i < _max_lights; ++i )
    {
        std::string uni_var = "lights[";
        uni_var += std::to_string( i );
        uni_var += "].";
        _per_vertex_state_set->addUniform( new osg::Uniform( ( uni_var + "intensity" ).c_str(), 10.f ) );
    }
    _per_vertex_state_set->addUniform( new osg::Uniform( "ambient_color", _ambient ) );
    ...
    _per_vertex_one_sided_program = new osg::Program;
    _per_vertex_one_sided_program->setName( "per_vertex_one_sided_lighting_shader" );
    _per_vertex_one_sided_vert_shader = new osg::Shader( osg::Shader::VERTEX, lighting_vert_one_sided_source );
    _per_vertex_one_sided_program->addShader( _per_vertex_one_sided_vert_shader );
    _per_vertex_one_sided_program->addShader( _per_vertex_frag_shader );

    _per_vertex_one_sided_state_set = new osg::StateSet;
    _per_vertex_one_sided_state_set->addUniform( new osg::Uniform( "numLights", 8 ) );
    for ( int i = 0; i < _max_lights; ++i )
    {
        std::string uni_var = "lights[";
        uni_var += std::to_string( i );
        uni_var += "].";
        _per_vertex_one_sided_state_set->addUniform( new osg::Uniform( ( uni_var + "intensity" ).c_str(), 10.f ) );
    }
    _per_vertex_one_sided_state_set->addUniform( new osg::Uniform( "ambient_color", _ambient ) );
    _per_vertex_one_sided_state_set->addUniform( new osg::Uniform( "eye_pos", osg::Vec4( 0.f, 0.f, 0.f, 1.f ) ) );
    _per_vertex_one_sided_state_set->setAttributeAndModes( _per_vertex_one_sided_program,
                                                 osg::StateAttribute::ON |
                                                 osg::StateAttribute::OVERRIDE );

    _per_pixel_program = new osg::Program;
    _per_pixel_program->setName( "per_pixel_lighting_shader" );
    ...
    _per_pixel_state_set->addUniform( new osg::Uniform( "numLights", 8 ) );
    for ( int i = 0; i < _max_lights; ++i )
    {
        std::string uni_var = "lights[";
        uni_var += std::to_string( i );
        uni_var += "].";
        _per_pixel_state_set->addUniform( new osg::Uniform( ( uni_var + "intensity" ).c_str(), 10.f ) );
    }
    _per_pixel_state_set->addUniform( new osg::Uniform( "ambient_color", _ambient ) );
    ...
    _per_pixel_one_sided_program = new osg::Program;
    _per_pixel_one_sided_program->setName( "per_pixel_lighting_shader" );
    _per_pixel_one_sided_vert_shader = new osg::Shader( osg::Shader::VERTEX, lighting_vert_source_pixel );
    _per_pixel_one_sided_frag_shader = new osg::Shader( osg::Shader::FRAGMENT, lighting_frag_one_sided_source_pixel );
    _per_pixel_one_sided_program->addShader( _per_pixel_one_sided_vert_shader );
    _per_pixel_one_sided_program->addShader( _per_pixel_one_sided_frag_shader );

    _per_pixel_one_sided_state_set = new osg::StateSet;
    _per_pixel_one_sided_state_set->addUniform( new osg::Uniform( "numLights", 8 ) );
    for ( int i = 0; i < _max_lights; ++i )
    {
        std::string uni_var = "lights[";
        uni_var += std::to_string( i );
        uni_var += "].";
        _per_pixel_one_sided_state_set->addUniform( new osg::Uniform( ( uni_var + "intensity" ).c_str(), 10.f ) );
    }
    _per_pixel_one_sided_state_set->addUniform( new osg::Uniform( "ambient_color", _ambient ) );
    _per_pixel_one_sided_state_set->addUniform( new osg::Uniform( "eye_pos", osg::Vec4( 0.f, 0.f, 0.f, 1.f ) ) );
    _per_pixel_one_sided_state_set->setAttributeAndModes( _per_pixel_one_sided_program,
                                                osg::StateAttribute::ON |
                                                osg::StateAttribute::OVERRIDE );
...
Name_List
Scene_Model::start_objects() const
{
    Name_List result;
...
Name_List
Scene_Model::segments() const
{
    Name_List result;

    for ( Segment_Map::const_iterator it = _segments[ _active_model ].cbegin();
            it != _segments[ _active_model ].cend(); ++it )
...
Scene_Model::change_shader( int value )
{
    ...
    int old_shader = _current_shader;
...
Scene_Model::set_ambient_color( osg::Vec4 amb )
{
    ...
    if ( _root->getNumChildren() > 0 )
    {
        ...
        // case PER_VERTEX_ONE_SIDED:
        current_scene()->getChild( PER_VERTEX_ONE_SIDED )->getOrCreateStateSet()->getUniform( "ambient_color" )->set( amb );

        // case PER_PIXEL:
        current_scene()->getChild( PER_PIXEL )->getOrCreateStateSet()->getUniform( "ambient_color" )->set( amb );

        // case PER_PIXEL_ONE_SIDED:
        current_scene()->getChild( PER_PIXEL_ONE_SIDED )->getOrCreateStateSet()->getUniform( "ambient_color" )->set( amb );
    }

}

void
Scene_Model::set_diffuse_color( osg::Vec4 diff, size_t index )
{
    get_lights()[ index ]->getLight()->setDiffuse( diff );

    // _active_model is non-negative if scene has been loaded
    if ( _root->getNumChildren() > 0 )
        _light_marker[_active_model][index]->getChild( 0 )->getOrCreateStateSet()->setAttribute( createSimpleMaterial( diff ) );
}

void
Scene_Model::set_diffuse_power( float d_power, size_t index  )
{
    // _active_model is non-negative if scene has been loaded
    if ( _root->getNumChildren() > 0 )
    {
        std::string uni_var = "lights[";
        uni_var += std::to_string( index );
        uni_var += "].";
        current_scene()->getChild( PER_VERTEX )->getOrCreateStateSet()->getUniform( ( uni_var + "intensity" ).c_str() )->set( d_power );
        current_scene()->getChild( PER_VERTEX_ONE_SIDED )->getOrCreateStateSet()->getUniform( ( uni_var + "intensity" ).c_str() )->set( d_power );
        current_scene()->getChild( PER_PIXEL )->getOrCreateStateSet()->getUniform( ( uni_var + "intensity" ).c_str() )->set( d_power );
        current_scene()->getChild( PER_PIXEL_ONE_SIDED )->getOrCreateStateSet()->getUniform( ( uni_var + "intensity" ).c_str() )->set( d_power );
    }
    _power = d_power;
}

void
Scene_Model::set_attenuation( float constant, float linear, float quad )
{
   _constant_attenuation = constant /10.f;
   _linear_attenuation = linear / 100.f;
   _quadratic_attenuation = quad / 100.f;

   if ( constant == 0.f && linear == 0.f && quad == 0.f )
      _constant_attenuation = 1.f;
}
...
Scene_Model::set_eye_position( osg::Vec3d center )
{
    ...
    {
        case OSG_DEFAULT: // OSG Default
            break;
        case PER_VERTEX: // Per Vertex
            uniform = _per_vertex_state_set->getUniform( "eye_pos" );
            uniform->set( osg::Vec4f( center, 1.f ) );
            break;
        case PER_VERTEX_ONE_SIDED: // Per Vertex
            uniform = _per_vertex_one_sided_state_set->getUniform( "eye_pos" );
            uniform->set( osg::Vec4f( center, 1.f ) );
            break;
        case PER_PIXEL: // Per Pixel
            uniform = _per_pixel_state_set->getUniform( "eye_pos" );
            uniform->set( osg::Vec4f( center, 1.f ) );
            break;
        case PER_PIXEL_ONE_SIDED: // Per Pixel
            uniform = _per_pixel_one_sided_state_set->getUniform( "eye_pos" );
            uniform->set( osg::Vec4f( center, 1.f ) );
            break;
    }
}
...
Scene_Model::update_model_index()
{
    ...
    _light_group.push_back( new osg::Group );
    std::vector< osg::MatrixTransform* > lg;
    _light_marker.push_back( lg );
    std::vector< osg::LightSource* > lls;
    _light_source.push_back( lls );
    std::vector< osg::MatrixTransform* > lpat;
    _light_transform.push_back( lpat );
    ...
    Segment_Map segs;
    _segments.push_back( segs );
    std::map<std::string, bool> seg_status;
    _segment_status.push_back( seg_status );
...
Scene_Model::show_invisible_objects( bool yes_or_no )
{
    ...
    if ( yes_or_no )
    {
        ...
        for ( it = _light_marker[_active_model].begin(); it != _light_marker[_active_model].end(); it++ )
            ( *it )->setNodeMask( ~INVISIBLE_MASK );
    }
    else
    {
...
Scene_Model::load_collada( QString& filename )
{
    ...
    else
    {
        ...
        launchHandlerAttachPoint = new osg::Group;
        ...
        for ( itn = _cv.back()->others.begin() ; itn < _cv.back()->others.end(); itn++ )
        {
            ...
            if ( body )
            {
               Motion_Data *md = new Motion_Data;
               md->body = body;
               itn->second->setUserData( (osg::Referenced *)md );
        ...
        for ( itn = _cv.back()->nodes.begin() ; itn < _cv.back()->nodes.end(); itn++ )
        {
            ...
            {
                boost::regex e( "_segment.*_$" );
                boost::smatch what;

                if ( boost::regex_match( itn->second->getName(), what, e ) )
                {
                    std::cout << "--------------" << std::endl;
                    std::cout << itn->second->getName() << std::endl;
                    std::cout << "--------------" << std::endl;
                    std::vector< btRigidBody* > bodies = process_segment( itn );
                    _segments.back()[itn->second->getName() ] = bodies;
                    _segment_status.back()[itn->second->getName() ] = true;
                    std::vector< btRigidBody* >::iterator it;
                    for ( it = bodies.begin(); it != bodies.end(); it++ )
                        add_rigid_body( *it, COL_REPORTABLE, _reportable_collides_with );
                    _segments_set.back() = true;
                }
                else
                {
                    ...
                    else if ( name.find( "_camera_block_pm_" ) != std::string::npos )
                    {
                        Physics_State* ps = ( Physics_State* )( body->getUserPointer() );
                        ps->reportable = true;
                        ps->camera = true;
        ...
        if ( _dynamics_world[ _active_model ]->has_crossbar && ( has_segments() || !has_path() ) )
        ...
        if ( has_path() )
        {
            ...
            {
                std::cerr << "Error! Collada file has both a path and segments ignoring the path" << std::endl;
                _path_set.back() = false;
            }
        }
        ...
        if ( _cv.back()->lights.size() > 0 )
        {
            for ( size_t light = 0; light < _cv.back()->lights.size(); light++ )
            {
                _light_group.back()->addChild( _cv.back()->lights[ light ].second );
            }
        }
        else
            create_lights( _scene_bound.back(), root_state_set ) ;
        root->addChild( _light_group.back() );
        ...
        if ( _camera_body[ _active_model ] != 0 )
        {
            if ( has_segments() )
            {
                CameraSegmentUpdateCallback* callback = new CameraSegmentUpdateCallback( _camera_body[ _active_model ],
                        _dynamics_world[ _active_model ],
                        _tracker[ _active_model ],
                        *_contactCallback,
                        _start_cross.back()[0] );
                callback->update_segment_mapping( _segment_status.back() );
                _camera_callback.back() = callback;
            }
            else if ( has_path() )
            {
            ...
            if ( _has_channel )
                ...
                // Segments can use the default movement callback
...
Scene_Model::switch_scene( int id )
{
    ...
    if ( _camera_body[ _active_model ] != 0 )
    {
        ...
        {
            _camera_callback[id] = new CameraSegmentUpdateCallback( _camera_body[ id ],
                                                                    _dynamics_world[ id ],
                                                                    _tracker[ id ],
                                                                    *_contactCallback,
                                                                    _start_cross[id][0] );
...
Scene_Model::initialize_physics()
{
    ...
    dynamicsWorld->setGravity( btVector3( 0, 0, -9.8 * _scale_factor ) );
...
Scene_Model::create_scene_graph( osg::Group* root )
{
    ...
    g = new osg::Group;
    g->setStateSet( _per_vertex_one_sided_state_set );
    g->addChild( root );
    switch_shader->addChild( g );
    ...
    g = new osg::Group;
    g->setStateSet( _per_pixel_one_sided_state_set );
    g->addChild( root );
    switch_shader->addChild( g );
...
{
    osg::Vec3f lower_bound = bb->corner( 4 );
    osg::Vec3f upper_bound = bb->corner( 7 );
    float x_offset = ( upper_bound.x() - lower_bound.x() );
    float y_offset = ( upper_bound.y() - lower_bound.y() );
    float height = upper_bound.z();

    if ( _light_marker[ _active_model ].size() > 0 )
    {
        _light_marker[ _active_model ].clear();
        _light_source[ _active_model ].clear();
        _light_transform[ _active_model ].clear();
    }
    ...
    for ( int k = 0; k < 2; k++ )
    {
       for ( int i = 0; i < 2; i++ )
       {
          for ( int j = 0; j < 2; ++j )
          {
             ...
             mat.makeTranslate( lower_bound.x() + i * x_offset,
                                lower_bound.y() + j * y_offset, height );

             osg::Geode *geode = new osg::Geode;
             geode->addDrawable( new osg::ShapeDrawable(
                 new osg::Sphere( osg::Vec3(), 0.01f * bb->radius() ) ) );
             geode->getOrCreateStateSet()->setAttribute(
                 createSimpleMaterial( osg::Vec4( 0.1f, 0.1f, 0.1f, 1.0f ) ) );
             osg::MatrixTransform *marker_xform = new osg::MatrixTransform();
             if ( !_show_invisibles )
                marker_xform->setNodeMask( INVISIBLE_MASK );
             marker_xform->addChild( geode );
             marker_xform->setMatrix( mat );
             _light_marker[ _active_model ].push_back( marker_xform );

             osg::MatrixTransform *light_xform = new osg::MatrixTransform();
             light_xform->setMatrix( mat );
             _light_transform[ _active_model ].push_back( light_xform );

             // create a spot light.
             osg::Light *myLight1 = new osg::Light;
             myLight1->setLightNum( light_index );
             myLight1->setPosition( osg::Vec4( 0.f, 0.f, 0.f, 1.0f ) );
             myLight1->setAmbient( _ambient );
             myLight1->setDiffuse( _diffuse );
             ...
             light_xform->addChild( lightS1 );

             _light_group[ _active_model ]->addChild( light_xform );
             _light_group[ _active_model ]->addChild( marker_xform );

             light_index++;
          }
       }

       osg::Vec3f diag = upper_bound - lower_bound;
       float dist = diag.length() / 4.;
       diag.normalize();
       lower_bound = lower_bound + diag * dist;
       upper_bound = upper_bound - diag * dist;
       x_offset = ( upper_bound.x() - lower_bound.x() );
       y_offset = ( upper_bound.y() - lower_bound.y() );
    }
}
                              ...
                              Physics_State* pState, bool substitute_sphere )
{
    ...
    if ( boost::regex_match( itn->second->getName(), what, e ) )
    {
        ...
        if ( pState->movable )
        {
            osg::Geode* geode = ( osg::Geode* )itn->second->getChild( 0 );
            osg::Drawable* drawable = geode->getDrawable( 0 );
            osg::BoundingBoxf bbox = drawable->getBoundingBox();
            osg::Vec3f len = bbox.corner(7) - bbox.corner(0);
            mass = len.x() * len.y() * len.z();
...
Scene_Model::process_segment( std::vector< Node_Offset_Pair >::iterator& itn )
{
    float dist;
    ...
    for ( uint32_t i = 0; i < vertices->getNumElements(); i++ )
    {
        ...
        else // Check for inline vertices
        {
            osg::Vec3f v1 = v - pts[count - 1];
            v1.normalize();
            osg::Vec3f v2 = pts[count - 1] - pts[count - 2];
            v2.normalize();
            if ( v1 * v2 < .9995 ) // i.e. > ~2 degrees
            {
                pts.push_back( v );
                count++;
            }
            else
                // Remove inline vertex
                pts.back() = v;
        }
    }
    ...
    for ( uint32_t i = 0; i < pts.size() - 1; i++ )
    {
        ...
        osgbDynamics::MotionState* motion = new osgbDynamics::MotionState();
        btVector3 btPt[2];
        btPt[0] = btVector3( pts[i].x(), pts[i].y(), pts[i].z() );
        btPt[1] = btVector3( pts[i + 1].x(), pts[i + 1].y(), pts[i + 1].z() );
        btScalar radi[2] = { _outer, _outer };
        btCollisionShape* collision = new btMultiSphereShape( btPt, radi, 2 );
        // Compute the matrix to rotate the segment to the primary axis
        btVector3 seg_vector = ( btPt[1] - btPt[0] );
        btVector3 norm_seg_vector = seg_vector.normalized();
        // For now assume Z up
        osg::Vec3f vec( norm_seg_vector.getX(), norm_seg_vector.getY(), norm_seg_vector.getZ() );
        osg::Matrixf rot_matrix = osg::Matrixf::rotate( vec, osg::Vec3f( 1, 0, 0 ) ) ;

        Physics_State* pState = new Physics_State();
        pState->s_node = new Segment_Node;
        // Want the unnormalized version so we can check the endpoints
        pState->s_node->direction = osg::Vec3f( seg_vector.getX(), seg_vector.getY(), seg_vector.getZ() ) * rot_matrix;
        pState->s_node->norm_direction = osg::Vec3f( norm_seg_vector.getX(), norm_seg_vector.getY(), norm_seg_vector.getZ() );
        pState->s_node->local_to_world = osg::Matrixf::translate( -pts[i] ) * rot_matrix;
        pState->s_node->start_pt = pts[i] * pState->s_node->local_to_world;
        pState->s_node->end_pt = pts[i + 1] * pState->s_node->local_to_world;
        pState->s_node->inner = _inner;
        pState->s_node->outer = _outer;
        pState->segment = true;
        std::ostringstream oss;
        oss << i;
        // Create a unique name by appending the point index to the end of
        // the base name
        pState->s_node->base_name = itn->second->getName();
        pState->s_node->name = itn->second->getName() + "_" + oss.str() + "___";
...
Scene_Model::move_offset( float x, float y, float z )
{
    ...
    if ( _homeOffset[ _active_model ] != nullptr )
    {
        ...
        mat.makeTranslate( _offset.x(), _offset.y(), _offset.z() );
        _homeOffset[ _active_model ]->setMatrix( mat );
    }
}

void
Scene_Model::set_rotations( float x, float y, float z )
{
    osg::Group* g = dynamic_cast<osg::Group*>( current_scene()->getChild( PER_VERTEX ) );
    osg::MatrixTransform* xform = dynamic_cast<osg::MatrixTransform*>( g->getChild( 0 ) );

    if ( xform != nullptr )
    {
        osg::Matrixd mat, matx, maty, matz;

        matx = osg::Matrix::rotate( ( x / 180.f ) * M_PI, 1, 0, 0 );
        maty = osg::Matrix::rotate( ( y / 180.f ) * M_PI, 0, 1, 0 );
        matz = osg::Matrix::rotate( ( z / 180.f ) * M_PI, 0, 0, 1 );
        mat = matx * maty * matz;
        xform->setMatrix( mat );
    }
}
...
void
Scene_Model::move_to_position( float x, float y, float z, float deg )
{
    ...
    osg::Vec3d dir( 1, 0, 0 );
    float radians = ( deg / 180.f ) * M_PI;
    dir = osg::Matrix::rotate( radians, 0, 0, 1 ) * dir;
    osg::Vec3d pos( x, y, z );
    ...
    if ( _camera_callback[ _active_model ] )
    {
        ...
        assert( callback != 0 );
        callback->update_start_direction( dir );
    }

    _tracker[ _active_model ]->setHomePosition( pos,
                                                pos + dir,
                                                osg::Vec3f( 0, 0, 1 ) );
}
...
void
Scene_Model::add_interaction_handler( Viewing_Window_Qt* v)
{
    osgbInteraction::LaunchHandler* lh = new osgbInteraction::LaunchHandler(
        _dynamics_world.back()->world, launchHandlerAttachPoint, v->getCamera() );
    {
        ...
        lh->setInitialVelocity( 20. *_scale_factor );

        // Also add the proper collision masks
        lh->setCollisionFilters( COL_EE, _ee_collides_with );

        v->addEventHandler( lh );
    }
}
...
void
Scene_Model::move_object( std::string name, float x, float y, float z )
{
    std::vector< osg::MatrixTransform* > result;

    std::vector< Node_Offset_Pair >::iterator itn;
    for ( itn = _cv.back()->others.begin() ; itn < _cv.back()->others.end(); itn++ )
    {
        if ( itn->second->getName().find( name ) != std::string::npos )
            result.push_back( itn->second );
    }
    for ( itn = _cv.back()->nodes.begin() ; itn < _cv.back()->nodes.end(); itn++ )
    {
        if ( itn->second->getName().find( name ) != std::string::npos )
    ...
    osg::Matrixd mat = osg::Matrix::translate( x, y, z );
    result[0]->setMatrix( mat );
}

void
Scene_Model::offset_object( std::string name, float x, float y, float z )
{
    std::vector< osg::MatrixTransform* > result;
    ...
    osg::Matrixd mat = osg::Matrix::translate( x, y, z );
    result[0]->postMult( mat );
}

void
Scene_Model::set_alpha( std::string name, float alpha )
{
  std::vector< osg::MatrixTransform * > result;
  ...
  if ( result[ 0 ]->getChild( 0 )->asGeode() != 0 )
  {
    osg::Geode *geode = (osg::Geode *)result[ 0 ]->getChild( 0 );
    for ( uint32_t child = 0; child < geode->getNumDrawables(); child++ )
    {
      ...
      if ( colors )
      {
        float *c_data = (float *)( colors->getDataPointer() );
        for ( uint32_t i = 0; i < colors->getNumElements(); i++ )
        {
          c_data[ 4 * i + 3 ] = alpha;
        }
      }
      osg::StateSet *state = geom->getStateSet();
...
void
Scene_Model::apply_physics( std::string name, float x, float y, float z )
{
  std::vector< osg::MatrixTransform * > result;
  ...
  if ( result.size() > 0 )
  {
     Motion_Data *md = (Motion_Data *)result[ 0 ]->getUserData();
     if ( md->body )
     {
        btVector3 punch( x, y, z );
        md->body->setAngularVelocity( btVector3( 0.f, 0.f, 0.f ) );
        md->body->setLinearVelocity( punch );
     }
     else
        std::cout << name << " does not have physics" << std::endl;
  }
}

namespace bpy = boost::python;
void
Scene_Model::toggle_segments( bpy::list& seg_ids, bool on_or_off )
{
    typedef bpy::stl_input_iterator<std::string> iterator_type;

    if ( bpy::len( seg_ids ) == 0 ) // Globally set influence
    {
        for ( std::map<std::string, bool>::iterator it = _segment_status[_active_model].begin();
                it != _segment_status[_active_model].end(); it++ )
            it->second = on_or_off;
    }
    else
    {
        BOOST_FOREACH( const iterator_type::value_type & it,
                       std::make_pair( iterator_type( seg_ids ), // begin
                                       iterator_type() ) )  // end
        {
            if ( _segment_status[_active_model].count( it ) != 0 )
                _segment_status[_active_model][ it ] = on_or_off;
            else
                std::cout << "!!! segment \"" << it << "\" not found. !!!!" << std::endl;
        }
    }

    CameraSegmentUpdateCallback* callback = dynamic_cast<CameraSegmentUpdateCallback*>( ( _camera_callback[ _active_model ] ).get() );
    callback->update_segment_mapping( _segment_status[_active_model] );
...
BOOST_PYTHON_MODULE( Scene_Model )
{
    bpy::class_< Name_List >( "Name_List" )
    .def( bpy::vector_indexing_suite< Name_List >() );

    bpy::class_< Scene_Model, Scene_Model_Ptr, boost::noncopyable >( "Scene_Model", bpy::no_init )
    .def( "starts",  &Scene_Model::start_objects )
    .def( "objects",  &Scene_Model::objects )
    .def( "segments",  &Scene_Model::segments )
    .def( "segments",  &Scene_Model::toggle_segments )
    .def( "move",  &Scene_Model::move_object )
    .def( "offset",  &Scene_Model::offset_object )
    .def( "set_alpha",  &Scene_Model::set_alpha )
    .def( "apply_physics",  &Scene_Model::apply_physics )
    ;
}
...
wrap_scene_model()
{
    ...
    if ( PyImport_AppendInittab( "Scene_Model", PyInit_Scene_Model ) == -1 )
        throw std::runtime_error( "Failed to add Scene_Model to the interpreter's "
                                  "builtin modules" );
}
